...
- **Semantic keyword tagging (CLIP).** An optional, self-hosted image tagging capability. When an image is scanned it can return semantic **keywords** ("Christmas", "celebration", "beach") alongside faces and objects — themes/scenes that detection can't express. Keywords come from comparing a CLIP image embedding against a global, admin-managed, editable vocabulary. New: `clip` model type on the Models page (download/activate/hot-swap like face/object, ViT-B/32 + ViT-L/14, not required to operate); bulk vocabulary upload/download (deduplicated, hidden when no CLIP model is active); per-image vectors persisted so vocabulary/template edits re-tag the library cheaply (re-score, no re-encode) via background jobs; a `keywords` settings category (top-K, threshold, prompt template); keyword chips on the Test and Tag pages. API: `keywords` in detect responses (`?keywords=sync|async|off`), `POST /api/keywords`, `GET /api/images/{id}/keywords`, `GET /api/images/search?keyword=`, and admin `GET/PUT /api/keywords/vocabulary`. Runs in-process on the existing onnxruntime with a vendored CLIP BPE tokenizer — no new Python dependency and no extra service. CLIP weights/tokenizer/vocabulary are external assets (mount under `models/clip/<name>/` or set `ARGUS_CLIP_ASSET_BASE`); the feature is inert until a model is active.
